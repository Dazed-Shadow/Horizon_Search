// ---------------------------------------------------------------------------
// TESTIMONIALS — placeholder content, structured for easy real-quote swap.
//
// To replace with real SBA veteran success stories this weekend:
//   1. Visit https://www.sba.gov/about-sba/sba-newsroom/success-stories
//   2. Filter by "Veteran" — each story includes a direct quote, name, and business.
//   3. Copy the quote, name, company, state, and the story URL into a slot below.
//      Adding sourceUrl will render a "Read their story ↗" link automatically.
//
// For quotes from your own network:
//   Ask for: one sentence on what federal contracting has meant for their business.
//   Leave sourceUrl empty for personal network quotes.
//
// Certification codes: SDVOSB, VOSB, 8AN (8a), HZC (HUBZone), WOSB, SBA
// ---------------------------------------------------------------------------

export const testimonials = [
  {
    quote: "I had no idea contracts this size were reserved specifically for businesses like mine. Once I got my SDVOSB certification, everything changed — we went from chasing commercial work to having a real pipeline.",
    name: "James R.",
    company: "JR Technical Services LLC",
    state: "TX",
    certification: "SDVOSB",
    // sourceUrl: "", // add SBA story URL here when you have a real quote
  },
  {
    quote: "Sources Sought notices were the piece nobody told me about. Responding to three of them before the formal solicitation dropped got our name in front of the contracting officer early. We won that contract.",
    name: "Denise M.",
    company: "Meridian Logistics Group",
    state: "VA",
    certification: "VOSB",
    // sourceUrl: "",
  },
  {
    quote: "The biggest thing I wish someone had told me on day one: get your SAM.gov registration done before you do anything else. Everything else — certifications, proposals, teaming — depends on that being active.",
    name: "Carlos V.",
    company: "Valor Infrastructure Partners",
    state: "FL",
    certification: "SDVOSB",
    // sourceUrl: "",
  },
];

// ---------------------------------------------------------------------------
// FOUNDER NOTE — your personal voice, always visible (not part of the rotation).
// This is where you speak directly about why you built Horizon Search.
// Write it in your own words — authenticity matters more than polish here.
// ---------------------------------------------------------------------------
export const founderNote = {
  message: "Horizon Search exists because veterans have earned more than a thank-you — they've earned a fair shot at the opportunities their service helped protect. This tool is my small part in making that shot a little easier to find.",
  // Replace the line above with your own message before distributing the link.
  name: "Horizon Search",
  role: "Built for the veteran business community",
};
